...
int get_register_id(Address ptr, int k) {
    if (ptr.value == -2) return 4;
    if (ptr.value == -3) return 2;
    if (ptr.real != -1) return ptr.real;
    if (ptr.value == stack_pointer)  return 29;
    if (ptr.value == retad_pointer)  return 31;
    if (ptr.value == -3) return 2;
    
    return k;
}

int codegen_fetch_register(Address ptr, int k) {
    if (ptr.value == -2) return 4;    
    if (ptr.value == -3) return 2;
    if (ptr.real != -1)  return ptr.real;
...
void codegen_store_register(char* s, int reg) {
    if (reg == -2) 
 	printf("move $a0, %s\n", s);
    if (reg == -3) 
	printf("move $v0, %s\n", s);
...
void codegen_generate_mips(Quadruple tuple,int delta) {
	...
	if (s=="label") {
        	...
        	printf("%s $%d, %s%d\n", s.c_str(), src, labelname, tuple.arguments[1].value);
        		...
        		int src1 = codegen_fetch_register(tuple.arguments[1], 9);
        		if (tuple.arguments[2].type == ADDRESS_CONSTANT) {
            			printf("%s $%d, $%d, %d\n", s.c_str(), dest, src1, tuple.arguments[2].value);
        		} else {
            			int src2 = codegen_fetch_register(tuple.arguments[2], 10);
            			printf("%s $%d, $%d, $%d\n", s.c_str(), dest, src1, src2);
        		}
        		if (dest == 8) {
            			codegen_store_register("$8", tuple.arguments[0].value);
        		}
    	} else if (s=="neg"|| s=="not") {
        	int src = codegen_fetch_register(tuple.arguments[1], 9);
        	int dest = get_register_id(tuple.arguments[0], 8);
       		printf("%s $%d, $%d\n", s.c_str(), dest, src);
        	if (dest == 8) {
            		codegen_store_register("$8", tuple.arguments[0].value);
        	...
        	int src = codegen_fetch_register(tuple.arguments[0], 8);
       	 	int dest = codegen_fetch_register(tuple.arguments[1], 9);
...
void codegen_local() {
	...
	for (i = 0; i < IR.size(); ++i) {
		...
		codegen_generate_mips(IR[i],delta);
		if (op=="func") {
